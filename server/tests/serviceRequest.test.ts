...
describe("Service Request Management API", () => {
  ...
  it("does not allow protected service-request ownership fields to be mass-assigned", async () => {
    const response = await request(app)
      .post("/api/v1/service-requests")
      .set("Authorization", `Bearer ${employeeToken}`)
      .send({
        title: "Protected fields request",
        description: "Keep tenant and requester immutable",
        type: "VPN Access",
      });
    ...
    const update = await request(app)
      ...
      .send({
        ...
        approvedBy: otherAdminId,
      });

    expect(update.status).toBe(200);
    const stored = await ServiceRequest.findById(id).lean();
    expect(stored?.organizationId.toString()).toBe(organizationId);
    expect(stored?.requestedBy.toString()).toBe(employeeId);
